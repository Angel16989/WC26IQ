...
export function KnockoutContent() {
  ...
  useEffect(() => {
    Promise.all([apiClient.knockout(), apiClient.scenarios()])
      .then(([b, s]) => { setBracket(b); setScenariosData(s); })
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="text-center space-y-3">
          <div className="mx-auto h-12 w-12 rounded-full border-2 animate-spin" style={{ borderColor: "var(--secondary)", borderTopColor: "transparent" }} />
  ...
  if (error || !bracket) {
    return (
      <div className="wc-error-panel rounded-2xl p-6">
        <p className="font-semibold">Knockout data unavailable</p>
        <p className="wc-body text-sm mt-1">{error ?? "No bracket data."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ── Header ──────────────────────────────────────────────────── */}
      <section className="wc-panel rounded-[28px] p-8 relative overflow-hidden">
        <div className="pitch-lines" aria-hidden />
        <div className="stadium-glow" aria-hidden />
        <div className="relative z-10">
          <p className="wc-eyebrow text-xs font-semibold">⚡ Knockout Stage</p>
          <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">
            Tournament Bracket
          </h1>
          <p className="wc-body mt-2 text-sm">
            Real results for completed matches · Poisson DC model predictions for upcoming ones
          </p>
